...
#urls = ["https://open-access-tage.de/open-access-tage-2024-koeln/koeln/programm/dienstag-10092024",
...
def list_main_rooms_first(day):
    try:
        with open(shortcut + "-rooms.txt", 'r', encoding="utf-8") as room_file:
...
# either we scrape the programm from the conftool website
# (if available there), or directly from the oat website
if "conftool.org" in urls[0]:
    # scrape from conftool website
    response = requests.get(url=urls[0])
    soup = BeautifulSoup(response.content, 'html.parser')
    all_days = soup.select('[id^="session_table_"]')

    for day in all_days:
        id = day["id"]
        day_string = id[len("session_table_"):]
        conference_day = Day(date=datetime.strptime(day_string, "%Y-%m-%d"))
        conference.add_day(conference_day)

        list_main_rooms_first(conference_day)

        # for each day round it up to the new X00 for better distinguishable ids
        global_counter = (int(str(global_counter)[0])+1)*100

        all_time_slots = day.select('[id="'+id+'"] [id^="session_time"]')
        print("time slots:", len(all_time_slots))
        for slot in all_time_slots:
            slot_id = slot["id"]
            all_cells = slot.select('[id="'+slot_id+'"]>tr>td')
            #print(slot["id"], '[id="'+slot_id+'"]>tr>td')
            #print(len(all_cells))
            #print(all_cells[0].text)
            #print(all_cells)
            start_time, duration = extract_start_time(all_cells[0].text)
            if duration == "00:00":
                # define 2h as the  default length of any open session
                duration = "02:00"
            for cell in all_cells[1:]:
                ...
                #print("  id  ", new_id,  "|"+title+"|")
                if new_id in seen_ids:
                    print("Warning: Same id used multiple times:", new_id, title)
                else:
                    seen_ids.append(new_id)

                track = determine_track(title_short)

                room_link = cell.select('a[href^="https://www.conftool.org/oat2025/index.php?page=browseSessions&form_room="]')
                if room_link:
                    room = room_link[0].text
                else:
                    room = "tba"
                ...
                if session_url != "":
                    ...
                    main = session_soup.select('#inner_content table [class^="paper"]')
                    if len(main)>0:
                        content = str(main[0].parent.parent)
                        content = re.sub('<p class="paper_title">(.*)</p>', '<h2>\\1</h2>',content)
                        content = re.sub('<span class="paper_type">(.*)</span>', '',content)

                else:
                    # session info from schedule directly for abstract
                    session_info = cell.select(".session_info")
                    if len(session_info)>0:
                        content = str(session_info[0])
                        
            
                
                session_object = Event(
                    id=new_id,
                    date=datetime.fromisoformat(day_string + "T" + start_time + ":00+02:00"),
                    start=start_time,
                    duration=duration,
                    track=track,
                ...
                if room not in [r.name for r in conference_day.room_objects]:
                    conference_day.add_room(Room(name=room))
                for r in conference_day.room_objects:
                    if r.name == room:
                        r.add_event(session_object)
                        break

else:
    # scrape from oat website
    for url in urls:
...
if len(rooms_list)>0:
    with open(shortcut + "-rooms.txt", 'r+', encoding="utf-8") as outfile:
        # write the rooms only the first times to the file, but allow manual reordering within the file
        if len([r.strip() for r in outfile.readlines()]) != len(rooms_list):
